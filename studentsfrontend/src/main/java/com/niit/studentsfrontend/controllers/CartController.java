...
public class CartController {
	...
	public ModelAndView confirmOrder(){
		...
		System.out.println("Address = "+addrList.get(0));
		o.setUser(userDao.getUserById(p.getName()));
	
		System.out.println("User = "+userDao.getUserById(p.getName()));
	...
	public ModelAndView processOrder(@ModelAttribute("orderObj")Order orderObj){
		...
		System.out.println("Cart Id : "+cartDao.getCartByCustomer(email).getCartId());
		orderDao.makeOrder(orderObj);
		
		
		cartDao.deleteCart(cartDao.getCartByCustomer(email).getCartId());
